...
Shipments.findById = async (req) => {
  ...
  const querys = [
    ...
    `SELECT SUM(ds_bill_charge) AS Billed FROM [DBA].disp_ship WHERE YEAR(ds_bill_date) = YEAR(GETDATE()) and ds_status = 'W' ${req.body.id != "Admin" ? `and ds_billto_id = ${req.body.id}` : ""}`,
    `SELECT SUM(ds_bill_charge) AS Complete FROM [DBA].disp_ship WHERE ds_status IN('N','Q','T') ${req.body.id != "Admin" ? `and ds_billto_id = ${req.body.id}` : ""}`,
    `SELECT SUM(ds_bill_charge) AS ShipOpen FROM [DBA].disp_ship WHERE ds_status = 'K' ${req.body.id != "Admin" ? `and ds_billto_id = ${req.body.id}` : ""}`
  ]
