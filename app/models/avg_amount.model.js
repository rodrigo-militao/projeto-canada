...
Avg_amount.findById = async (req) => {
  ...
  const ontime_sql = `SELECT ds_id AS Ref_Num, ds_status, (SELECT co_name FROM [DBA].companies WHERE co_id = ds_billto_id) AS Customer, (SELECT top 1 de_apptdate FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC) AS appt_date, (SELECT top 1 de_appttime FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC) AS appt_time, (SELECT top 1 de_arrdate FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC) AS del_date, (SELECT top 1 de_arrtime FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC) AS del_time, CONVERT(DATETIME, CONVERT(CHAR(8), (SELECT top 1 de_apptdate FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC), 112) + ' ' + CONVERT(CHAR(8), (SELECT top 1 de_appttime FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC), 108)) AS appt_date_time, CONVERT(DATETIME, CONVERT(CHAR(8), (SELECT top 1 de_arrdate FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC), 112) + ' ' + CONVERT(CHAR(8), (SELECT top 1 de_arrtime FROM [DBA].disp_events WHERE de_shipment_id = ds_id and de_site = ds_findest_id and de_arrdate IS NOT NULL ORDER BY de_ship_seq ASC), 108)) AS del_date_time, ( IF del_date_time <= appt_date_time THEN 'On-Time' ELSE 'Out-Time' END IF) AS on_time FROM [DBA].disp_ship  WHERE (YEAR(del_date) = YEAR(GETDATE())) and (appt_date IS NOT NULL and del_date IS NOT NULL and appt_time IS NOT NULL and del_time IS NOT NULL) AND ds_status IN('K','N','Q','T','W') ${req.body.id != "Admin" ? `and ds_billto_id = ${req.body.id}` : ""} ORDER BY del_date DESC`
  ...
  const req_ontime = await fetch(options.url, {...options, body: JSON.stringify({sql: ontime_sql})})
  const data_ontime = await req_ontime.json()
