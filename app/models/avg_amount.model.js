...
Avg_amount.findById = async (req) => {
  ...
  const in_transit_sql = "select count(*) from [DBA].current_shipments where cs_completed < cs_event_count and cs_completed > 0 and (cc_origin_conf = 'T' or cc_findest_conf = 'T')"
  ...
  const data = {
    ...
    orders: data_orders,
    in_transit: data_in_transit[0]["count()"],
  }
